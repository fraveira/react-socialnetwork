...
module.exports.getUserById = id => {
    return db.query(
        `SELECT id, first, last, profilepicture, bio FROM users WHERE id = $1`,
        [id]
    );
};
...
module.exports.addProfilePic = (id, profilepicture) => {
    return db.query(
        `UPDATE users SET profilepicture=$2 WHERE id = $1 RETURNING profilepicture`,
        [id, profilepicture]
    );
};
...
module.exports.getLastThree = () => {
    return db.query(
        `SELECT id, first, last, profilepicture FROM users ORDER BY id DESC LIMIT 3`
    );
};

module.exports.getMatchingUsers = val => {
    return db.query(
        `SELECT id, first, last, profilepicture FROM users WHERE first ILIKE $1 LIMIT 3`,
        [val + "%"]
    );
};
